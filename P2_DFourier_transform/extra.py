...
reflect_top_left = (2*center[0]- (x+w), 2*center[1]- (y+h))
reflect_bot_right = (2*center[0]- x, 2*center[1]-y)

mask = np.ones(magnitude.shape[:2], dtype=np.uint8)
cv.rectangle(mask, top_left, bot_right, 0, -1)
cv.rectangle(mask, reflect_top_left, reflect_bot_right,  0 , -1)
